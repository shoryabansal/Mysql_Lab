...
(
SELECT
pro_info.Product_Name , pro_info.Product_Id
FROM
product_info AS pro_info
LEFT JOIN
category AS ctg
ON pro_info.Category_Id =ctg.Id
WHERE
LOWER(ctg.Name) LIKE '%golf%'
ORDER BY Product_id 
) as summary;
/*
...
LEFT JOIN
product_info AS pro_info
ON ord_itm.Item_Id=pro_info.Product_Id
LEFT JOIN
category AS ctg
ON pro_info.Category_Id =ctg.Id
WHERE
